...
def run_YS_by_thread(image_path, model_path, conf, output_dir):
    try:
        # טעינת המודל
        model = YOLO(model_path)

        results = model.predict(image_path, conf=conf, save=False , project=output_dir)
        ...
        txt_filename = f"yolo_signs_results_{base_image_name}.txt"
...
if __name__ == '__main__':
    #נתיב לתמונה
    image_path=r"C:\Users\USER\Desktop\2X8A0770.JPG"
    #נתיב למודל
    model_path=r"C:\Users\USER\Desktop\PROJECT\NEWYOLO\best.pt"
    #נתיב לתיקייה לשמירת התוצאות
    output_dir=r"C:\Users\USER\Desktop\PROJECT\sign_from_thread"
    #הגדרת סף הביטחון
    conf=0.4
    #הגדרת הקלאסים לזיהוי
    classes=[0]
    #זימון הפונקציה להרצת המודל
    run_YS_by_thread(image_path,model_path,conf,classes,output_dir)
